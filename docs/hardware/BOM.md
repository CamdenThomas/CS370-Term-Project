...
| 1 | OBDLink LX, Bluetooth Classic OBD2 adapter (STN chip) | ~$60, verify | The data path (D-019). Bluetooth Classic (SPP), **not BLE**. A cheap ELM327 Bluetooth clone works for the PID survey but is unreliable for a 48 h run. |
...
**Core subtotal ≈ $155** (verify the adapter's price when ordering).

**Fallback, not ordered:** a USB OBD2 adapter with an STN chip (e.g. OBDLink SX, ~$30,
D-015) if Bluetooth proves unreliable in the car.
