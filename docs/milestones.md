...
| M1 | Problem memo; each partner's `.jsonl` transcripts copied out | 5 | 2026-09-27 | 3–4 | 🟡 5/19 |
| M2 | Design document; both sensors electrically alive; **baseline collection starts** | 6–7 | 2026-10-11 | 8–10 | 🟡 2/21 |
...
- [x] Board live: `docs/BOARD/board.toml` → issues + the `carwatch` project (`docs/BOARD/BOARD.md`)
...
- [ ] **Three-isolated-workers check passes** on `docs/DESIGN.md`: a design-only
      reviewer, a parts-only buyer who never sees the design, and a builder who only
      follows instructions must each succeed from their own section alone
