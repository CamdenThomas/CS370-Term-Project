...
def render_managed(item: dict, numbers: dict[str, int],
    ...
    block = (
        f"{DEP_OPEN}\n\n---\n" + "\n\n".join(lines) +
        "\n\n<sub>Maintained by tools/board_sync.py from docs/BOARD/board.toml — edits here are overwritten.</sub>\n"
        f"{DEP_CLOSE}"
    )
...
def sync_milestone_doc(manifest: dict, items: dict, numbers: dict[str, int],
    ...
    block = (MS_OPEN + "\n\n" + "\n".join(rows) +
             "\n\n<sub>Generated by `tools/board_sync.py` from `docs/BOARD/board.toml`.\n"
